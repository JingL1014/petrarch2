	...
	for path in filepaths:
		...
		for event, elem in tree:
			if event == "end" and elem.tag == "Sentence":
				...
				attribute_check = [key in story.attrib for key in ['date', 'id', 'sentence', 'category']]
			...
			if 'parsed' in event_dict[key]['sents'][sent]:
				...
				for n in sentence.metadata['nouns']: 
					...
					for i in range(0,len(codes)):
						...
						print("code:"+str(codes[i])+"\tmatched text:"+matched_txt[i]+"\tmeaning:"+(("#").join(n.meaning) if n.meaning != None else '-'))
				#print('triplets:')
				#for triple in sentence.metadata['triplets']: print("s: "+triple[0]+"\tt: "+triple[1].text+"\tv: "+triple[2]+"\to: "+(" ").join(triple[3].text)+"\n")
				

				sentence.get_meaning()
				#print('verbs:')
